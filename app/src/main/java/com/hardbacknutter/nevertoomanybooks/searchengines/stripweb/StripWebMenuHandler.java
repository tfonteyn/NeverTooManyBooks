...
public class StripWebMenuHandler
        extends SiteSearchMenuHandler {

    /**
     * Constructor.
     */
    public StripWebMenuHandler() {
        super(R.menu.sm_search_on_stripweb,
              R.id.SUBMENU_STRIPWEB_SEARCH,
              R.id.MENU_STRIPWEB_BOOKS_BY_AUTHOR,
              R.id.MENU_STRIPWEB_BOOKS_BY_AUTHOR_IN_SERIES,
              R.id.MENU_STRIPWEB_BOOKS_IN_SERIES);
    }

    @Override
    public boolean isShowMenu(@NonNull final Context context) {
        final String key = EngineId.StripWebBe.getPreferenceKey()
    ...
    protected void startSearchActivity(@NonNull final Context context,
        ...
        if (author != null) {
            ...
            if (!cAuthor.isEmpty()) {
                try {
                    fields.add(URLEncoder.encode(cAuthor, StripWebSearchEngine.CHARSET));
        ...
        if (series != null) {
            ...
            if (!cSeries.isEmpty()) {
                try {
                    fields.add(URLEncoder.encode(cSeries, StripWebSearchEngine.CHARSET));
                } catch (@NonNull final UnsupportedEncodingException ignore) {
                    // ignore
                }
            }
        ...
        final String url = EngineId.StripWebBe.requireConfig().getHostUrl(context)
                           + String.format(StripWebSearchEngine.SEARCH_URL,
                                           fields);
