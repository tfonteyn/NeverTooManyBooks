 ...
 * </ul>
 */
// FIXME: 27/09/2024 we need to unify the update-handling for the TOC
//  and the ReadStatus/Progress fragments
            ...
            for (int cIdx = 0; cIdx < DBKey.NR_OF_BOOK_COVERS; cIdx++) {
                if (style.isShowField(FieldVisibility.Screen.Detail, DBKey.COVER[cIdx])) {
                    ...
                    imageHandler[cIdx] = new ImageHandler
                            ...
                            .setCoverBrowserCodeSupplier(() -> vm.getBook().getRawProductCode())
                            .build();
                }
            }
        } finally {
