...
//						+ " LEFT OUTER JOIN " + DB_TB_SERIES + " s ON (s." + KEY_ID + "=w." + KEY_SERIES_ID + ") ";
	 ...
	 */
	@SuppressWarnings("WeakerAccess")
	public String fetchAllBooksInnerSql(@Nullable final String order, @NonNull final String bookshelf,
	 ...
	 */
	public int fetchAuthorPositionByGivenName(@NonNull final String name, @NonNull final String bookshelf) {
		...
		String sql = "SELECT count(*) as count FROM " + DB_TB_AUTHORS + " a " +
			"WHERE ( " + makeTextTerm("a." + DOM_AUTHOR_GIVEN_NAMES, "<", names[0]) +
			"OR ( " + makeTextTerm("a." + DOM_AUTHOR_GIVEN_NAMES, "=", names[0]) +
			"     AND " + makeTextTerm("a." + DOM_AUTHOR_FAMILY_NAME, "<", names[1]) + ")) " +
			where + 
	 ...
	 * @return The position of the author
	 */
	public int fetchAuthorPositionByName(@NonNull final String name, @NonNull final String bookshelf) {
		...
		String sql = "SELECT count(*) as count FROM " + DB_TB_AUTHORS + " a " +
			"WHERE ( " + makeTextTerm("a." + DOM_AUTHOR_FAMILY_NAME, "<", names[0]) +
			"OR ( " + makeTextTerm("a." + DOM_AUTHOR_FAMILY_NAME, "=", names[0]) +
			"     AND " + makeTextTerm("a." + DOM_AUTHOR_GIVEN_NAMES, "<", names[1]) + ")) " +
			where + 
...
//	}
	...
	@SuppressWarnings("WeakerAccess")
	public long fetchBookshelfIdByName(@NonNull final String name) {
	...
	public long createAnthologyTitle(long book, String author, @NonNull final String title,
                                     boolean returnDupId, boolean dirtyBookIfNecessary) {
		if (!title.isEmpty()) {
	 ...
	 */
	@SuppressWarnings("WeakerAccess")
	public long createAnthologyTitle(long book, long authorId, @NonNull final String title,
                                     boolean returnDupId, boolean dirtyBookIfNecessary) {
		if (!title.isEmpty()) {
	 ...
	 */
	private long createAuthor(@NonNull final String family_name, @NonNull final String given_names) {
		ContentValues initialValues = new ContentValues();
		initialValues.put(DOM_AUTHOR_FAMILY_NAME.name, family_name);
		initialValues.put(DOM_AUTHOR_GIVEN_NAMES.name, given_names);
	 ...
	 */
	private long createSeries(@NonNull final String seriesName) {
		ContentValues initialValues = new ContentValues();
	 ...
	 */
	private void createBookshelfBooks(long bookId, @NonNull final ArrayList<String> bookshelves,
	 ...
	 */
	public boolean updateAnthologyTitle(long rowId, long book, @NonNull final String author, @NonNull final String title, boolean dirtyBookIfNecessary) {
	...
	private String getAuthorId(@NonNull final String name) {
		String[] names = processAuthorName(name);
	...
	private long getAuthorId(@NonNull final String[] names) {
		if (mGetAuthorIdStmt == null) {
			mGetAuthorIdStmt = mStatements.add(
			        "mGetAuthorIdStmt", "Select " + DOM_ID + " From " + DB_TB_AUTHORS +
	...
	private String getAuthorIdOrCreate(@NonNull final String[] names) {
	...
	public long lookupAuthorId(@NonNull final Author a) {
		return getAuthorId(new String[] {a.familyName, a.givenNames});
	}

	public long lookupSeriesId(@NonNull final Series s) {
		return getSeriesId(s.name);
	}

	public long lookupAnthologyTitleId(@NonNull final AnthologyTitle a) {
	...
	private void preprocessOutput(boolean isNew, @NonNull final BookData values) {
		...
		if (values.containsKey(DOM_AUTHOR_FORMATTED.name)) {
			authorId = getAuthorId(values.getString(DOM_AUTHOR_FORMATTED.name));
...
//	private static final String NEXT_STMT_NAME = "next";
	private void createBookAnthologyTitles(long bookId, @NonNull final ArrayList<AnthologyTitle> list,
										   @SuppressWarnings("SameParameterValue") boolean dirtyBookIfNecessary) {
		if (dirtyBookIfNecessary)
			setBookDirty(bookId);
...
//		SynchronizedStatement stmt = mStatements.get(NEXT_STMT_NAME);
		...
		if (authors != null) {
			...
			for (Author author : authors) {
				...
				try {
					...
					authorIdStr = getAuthorIdOrCreate(new String[] {author.familyName, author.givenNames});
					long authorId = Long.parseLong(authorIdStr);
					...
					Logger.logError(e);
					throw new RuntimeException("Error adding author '" + author.familyName + "," + author.givenNames + "' {" + authorIdStr + "} to book " + bookId + ": " + e.getMessage(), e);
				}
			}
		}
	}
	 ...
	 */
	@SuppressWarnings("WeakerAccess")
	public void syncAuthor(@NonNull final Author a) {
	 ...
	 */
	public void sendAuthor(@NonNull final Author a) {
		if (a.id == 0) {
	 ...
	 */
	private void addOrUpdateAuthor(@NonNull final Author a) {
		if (a.id != 0) {
			// Get the old author
			...
			if (!a.familyName.equals(oldA.familyName) || a.givenNames.equals(oldA.givenNames)) {
				...
				v.put(DOM_AUTHOR_FAMILY_NAME.name, a.familyName);
				v.put(DOM_AUTHOR_GIVEN_NAMES.name, a.givenNames);
	 ...
	 * @param a		Author in question
	 */
	public void refreshAuthor(@NonNull final Author a) {
		if (a.id == 0) {
	 ...
	 */
	@SuppressWarnings("WeakerAccess")
	public void syncSeries(@NonNull final Series s) {
...
//////////////////////////////////////////
	 ...
	 */
	@SuppressWarnings("WeakerAccess")
	public BooksCursor fetchBooks(@NonNull final String sql, @NonNull final String[] selectionArgs) {
...
/**************************************************************************************/
	 ...
	 */
	@SuppressWarnings("WeakerAccess")
	public void ftsSendBooks(@NonNull final BooksCursor books, @NonNull final SynchronizedStatement stmt) {
