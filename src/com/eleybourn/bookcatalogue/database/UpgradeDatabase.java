...
public final class UpgradeDatabase {
    ...
    static void recreateAndReloadTable(@NonNull final DbSync.SynchronizedDb db,
                                       @SuppressWarnings("SameParameterValue") @NonNull final TableDefinition tableToRecreate,
                                       @NonNull final String... toRemove) {
    ...
    private static void v72_renameIdFilesToHash(@NonNull final DbSync.SynchronizedDb db) {
        try (Cursor c = db.rawQuery("SELECT " + DOM_PK_ID + ',' + DOM_BOOK_UUID +
                                        " FROM " + TBL_BOOKS + " ORDER BY " + DOM_PK_ID, null)) {
            while (c.moveToNext()) {
    ...
    private static void v74_fixupPositionedBookItems(@NonNull final DbSync.SynchronizedDb db,
        ...
        String sql = "SELECT " + TBL_BOOKS.dotAs(DOM_PK_ID) + ',' +
            ...
            " FROM " + TBL_BOOKS.ref() + " JOIN " + tableName + " o ON o." + DOM_FK_BOOK_ID + '=' + TBL_BOOKS.dot(
            DOM_PK_ID) +
        ...
        String updateSql = "UPDATE " + tableName + " SET " + positionField + "=1 WHERE " + DOM_FK_BOOK_ID + "=? AND " + positionField + "=?";
     ...
     * This routine renames all files, if they exist.
     */
    static void v200_moveCoversToDedicatedDirectory(@NonNull final DbSync.SynchronizedDb db) {
    ...
    static int doUpgrade(@NonNull final SQLiteDatabase db,
        ...
        if (curVersion == 25) {
            curVersion++;
            mMessage += "* Your sort order will be automatically saved when closing the application (Requested by Martin)\n\n";
        ...
        if (curVersion == 32) {
            ...
            try {
                ...
                db.execSQL(
                    "ALTER TABLE " + TBL_BOOKS + " ADD " + DOM_BOOK_ANTHOLOGY_BITMASK + " int not null default 0");
            } catch (Exception e) {
                Logger.error(e);
                throw new RuntimeException("Failed to upgrade database", e);
            }
        ...
        if (curVersion == 34) {
            ...
            try {
                ...
                db.execSQL(
                    "ALTER TABLE " + TBL_BOOKS + " ADD " + OLD_KEY_AUDIOBOOK + " boolean not null default 'f'");
                db.execSQL(
                    "ALTER TABLE " + TBL_BOOKS + " ADD " + DOM_BOOK_SIGNED + " boolean not null default 'f'");
            } catch (Exception e) {
                Logger.error(e);
                throw new RuntimeException("Failed to upgrade database", e);
            }
        ...
        if (curVersion == 38) {
            curVersion++;
            db.execSQL(
                "DELETE FROM " + TBL_LOAN + " WHERE (" + DOM_LOANED_TO + "='' OR " + DOM_LOANED_TO + "='null')");
        }
        if (curVersion == 39) {
            curVersion++;
        }
