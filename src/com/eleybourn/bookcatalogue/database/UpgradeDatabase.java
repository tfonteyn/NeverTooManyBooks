...
import static com.eleybourn.bookcatalogue.database.DBHelper.DATABASE_CREATE_BOOKSHELF_DATA;
import static com.eleybourn.bookcatalogue.database.DBHelper.DATABASE_CREATE_BOOK_AUTHOR;
import static com.eleybourn.bookcatalogue.database.DBHelper.DATABASE_CREATE_BOOK_SERIES;
...
public final class UpgradeDatabase {
    ...
    static int doUpgrade(@NonNull final SQLiteDatabase db,
        ...
        if (curVersion == 22) {
            ...
            mMessage += "* The new comments tab also includes a notes field where you can add " +
                    "personal notes for any book (Requested by Luke).\n\n";
            mMessage += "* The new loaned books tab allows you to record books loaned to" +
                    " friends. This will lookup your phone contacts to pre-populate the list " +
                    "(Requested by Luke)\n\n";
            mMessage += "* Scanned books that already exist in the database (based on ISBN)" +
                    " will no longer be added (Identified by Colin)\n\n";
            mMessage += "* After adding a book, the main view will now scroll to a appropriate" +
                    " location. \n\n";
        ...
        if (curVersion == 25) {
            curVersion++;
            mMessage += "* Your sort order will be automatically saved when closing the" +
                    " application (Requested by Martin)\n\n";
            mMessage += "* There is a new 'about this app' view available from the " +
                    "administration tabs (Also from Martin)\n\n";
        }
        if (curVersion == 26) {
            curVersion++;
            mMessage += "* There are two additional sort functions, by series and by loaned" +
                    " (Request from N4ppy)\n\n";
            mMessage += "* Your bookshelf and current location will be saved when you exit " +
        ...
        if (curVersion == 27) {
            ...
            mMessage += "* The exports have been renamed to csv, use the correct published" +
        ...
        if (curVersion == 29) {
            ...
            mMessage += "* You can bulk update the thumbnails for all books with ISBN's from " +
                    "the Admin page\n\n";
        }
        if (curVersion == 30) {
            curVersion++;
            mMessage += "* You can now delete individual thumbnails by holding on the image and" +
                    " selecting delete.\n\n";
        }
        if (curVersion == 31) {
            curVersion++;
            ...
            mMessage += "* When adding books the bookshelf will default to your currently" +
        ...
        if (curVersion == 32) {
            ...
            mMessage += "* There is now support to record books as anthologies and it's " +
                    "titles.\n\n";
            mMessage += "* There is experimental support to automatically populate the" +
                    " anthology titles \n\n";
            mMessage += "* You can now take photos for the book cover (long click on the " +
                    "thumbnail in edit) \n\n";
        }
        if (curVersion == 33) {
            curVersion++;
            ...
            mMessage += "* Thumbnails can now be hidden just like any other field" +
                    " (Thanks Martin)\n\n";
            mMessage += "* You can also rotate thumbnails; useful for thumbnails taken with" +
                    " the camera\n\n";
            mMessage += "* Bookshelves will appear in the menu immediately" +
                    " (Thanks Martin/Julia)\n\n";
        }
        if (curVersion == 34) {
            curVersion++;
        ...
        if (curVersion == 36) {
            ...
            mMessage += "*** Don't forget you can hide any of the new fields that you" +
                    " do not want to see.\n\n";
            ...
            mMessage += "* Fixed several defects for Android 1.6 users - I do not have a 1.6" +
                    " device to test on so please let me know if you discover any errors\n\n";
        }
        if (curVersion == 37) {
            curVersion++;
            mMessage += "Tip: If you long click on a book title on the main list you can" +
                    " delete it\n\n";
            mMessage += "Tip: If you want to see all books, change the bookshelf to" +
                    " 'All Books'\n\n";
            mMessage += "Tip: You can find the correct barcode for many modern paperbacks on" +
                    " the inside cover\n\n";
            mMessage += "* There is now a 'Sort by Unread' option, as well as a 'read' icon" +
                    " on the main list (requested by Angel)\n\n";
            mMessage += "* If you long click on the (?) thumbnail you can now select a new" +
                    " thumbnail from the gallery (requested by Giovanni)\n\n";
            mMessage += "* Bookshelves, loaned books and anthology titles will now import" +
                    " correctly\n\n";
        }
        if (curVersion == 38) {
            curVersion++;
        ...
        if (curVersion == 41) {
            curVersion++;
            mMessage += "Tip: You can find the correct barcode for many modern paperbacks" +
                    " on the inside cover\n\n";
            mMessage += "* Added app2sd support (2.2 users only)\n\n";
            mMessage += "* You can now assign books to multiple bookshelves" +
                    " (requested by many people)\n\n";
            mMessage += "* A .nomedia file will be automatically created which will stop the" +
                    " thumbnails showing up in the gallery (thanks Brandon)\n\n";
            mMessage += "* The 'Add Book by ISBN' page has been redesigned to be simpler and" +
            ...
            try {
                ...
                db.execSQL("INSERT INTO " + TBL_BOOK_BOOKSHELF + " (" +
                                   ...
                                   " SELECT " + DOM_PK_ID + ',' + DOM_FK_BOOKSHELF_ID + " FROM " + TBL_BOOKS);
                db.execSQL("CREATE TABLE tmp1 AS SELECT _id, " + OLD_KEY_AUTHOR + ',' +
                                   DOM_TITLE + ',' + DOM_BOOK_ISBN + ',' + DOM_BOOK_PUBLISHER + ',' +
                                   DOM_BOOK_DATE_PUBLISHED + ',' + DOM_BOOK_RATING + ',' + DOM_BOOK_READ + ',' +
                                   OLD_KEY_SERIES + ',' + DOM_BOOK_PAGES + ',' + DOM_BOOK_SERIES_NUM +
                                   ',' + DOM_BOOK_NOTES + ',' + DOM_BOOK_PRICE_LISTED + ',' +
                                   DOM_BOOK_ANTHOLOGY_BITMASK + ',' + DOM_BOOK_LOCATION + ',' +
                                   DOM_BOOK_READ_START + ',' + DOM_BOOK_READ_END + ',' +
                                   OLD_KEY_AUDIOBOOK + ',' + DOM_BOOK_SIGNED + " FROM " + TBL_BOOKS);
        ...
        if (curVersion == 43) {
            ...
            db.execSQL("DELETE FROM " + TBL_TOC_ENTRIES + " WHERE " + DOM_PK_ID + " IN (" +
                               "SELECT a." + DOM_PK_ID + " FROM " + TBL_TOC_ENTRIES + " a, " + TBL_TOC_ENTRIES + " b " +
                               " WHERE a." + DOM_FK_BOOK_ID + "=b." + DOM_FK_BOOK_ID + " AND a." + OLD_KEY_AUTHOR + "=b." + OLD_KEY_AUTHOR + ' ' +
                               " AND a." + DOM_TITLE + "=b." + DOM_TITLE + " AND a." + DOM_PK_ID + " > b." + DOM_PK_ID + "" +
                               ')');

            db.execSQL(
                    "CREATE UNIQUE INDEX IF NOT EXISTS anthology_pk_idx ON " + TBL_TOC_ENTRIES + " (" + DOM_FK_BOOK_ID + ',' + OLD_KEY_AUTHOR + ',' + DOM_TITLE + ')');
        }
        if (curVersion == 44) {
            curVersion++;
            ...
            db.execSQL("CREATE TABLE tmp1 AS SELECT _id, " + OLD_KEY_AUTHOR + ',' +
                               ...
                               DOM_BOOK_SIGNED + " FROM " + TBL_BOOKS);

            db.execSQL(
                    "CREATE TABLE tmp2 AS SELECT _id, " + DOM_FK_BOOK_ID + ',' + DOM_LOANED_TO + " FROM " + TBL_LOAN);
            db.execSQL(
                    "CREATE TABLE tmp3 AS SELECT _id, " + DOM_FK_BOOK_ID + ',' + OLD_KEY_AUTHOR + ',' + DOM_TITLE + ',' + DOM_BOOK_TOC_ENTRY_POSITION + " FROM " + TBL_TOC_ENTRIES);
            db.execSQL(
                    "CREATE TABLE tmp4 AS SELECT " + DOM_FK_BOOK_ID + ',' + DOM_FK_BOOKSHELF_ID + " FROM " + TBL_BOOK_BOOKSHELF);
        ...
        if (curVersion == 48) {
            ...
            db.execSQL(
                    "DELETE FROM loan WHERE _id!=(SELECT max(l2._id) FROM loan l2 WHERE l2.book=loan.book);");
            db.execSQL(
                    "DELETE FROM anthology WHERE _id!=(SELECT max(a2._id) FROM anthology a2 WHERE a2.book=anthology.book AND a2.author=anthology.author AND a2.title=anthology.title);");
        }
        if (curVersion == 49) {
            curVersion++;
        ...
        if (curVersion == 53) {
            ...
            if (go) {
                ...
                try {
                    ...
                    db.execSQL(
                            "INSERT INTO " + TBL_BOOK_SERIES + " (" + DOM_FK_BOOK_ID + ',' + DOM_FK_SERIES_ID + ',' + DOM_BOOK_SERIES_NUM + ',' + DOM_BOOK_SERIES_POSITION + ") "
                    ...
                    db.execSQL(
                            "INSERT INTO " + TBL_BOOK_AUTHOR + " (" + DOM_FK_BOOK_ID + ',' + DOM_FK_AUTHOR_ID + ',' + DOM_BOOK_AUTHOR_POSITION + ") "
                                    + "SELECT b." + DOM_PK_ID + ", b." + OLD_KEY_AUTHOR + ", 1 FROM " + TBL_BOOKS + " b ");

                    String tmpFields = DOM_PK_ID + "," /* + KEY_AUTHOR + "," */ + DOM_TITLE + ',' + DOM_BOOK_ISBN
                            + ',' + DOM_BOOK_PUBLISHER + ',' + DOM_BOOK_DATE_PUBLISHED + ',' + DOM_BOOK_RATING + ',' + DOM_BOOK_READ
                            + /* "," + KEY_SERIES + */ ',' + DOM_BOOK_PAGES /* + "," + KEY_SERIES_NUM */ + ',' + DOM_BOOK_NOTES
                            + ',' + DOM_BOOK_PRICE_LISTED + ',' + DOM_BOOK_ANTHOLOGY_BITMASK + ',' + DOM_BOOK_LOCATION + ',' + DOM_BOOK_READ_START
                            + ',' + DOM_BOOK_READ_END + ',' + DOM_BOOK_FORMAT + ',' + DOM_BOOK_SIGNED + ',' + DOM_BOOK_DESCRIPTION
                            + ',' + DOM_BOOK_GENRE;
                    db.execSQL(
                            "CREATE TABLE tmpBooks AS SELECT " + tmpFields + " FROM " + TBL_BOOKS);
                    ...
                    db.execSQL(
                            "INSERT INTO " + TBL_BOOKS + "( " + tmpFields + ")  SELECT * FROM tmpBooks");
            ...
            if (curVersion == 55) {
                ...
                // This is to check and skip as required
                boolean go2 = false;
                String checkSQL2 = "SELECT * FROM " + TBL_BOOKS;
                try (Cursor results = db.rawQuery(checkSQL2, null)) {
                    if (results.getCount() > 0) {
                        if (results.getColumnIndex(OLD_KEY_AUTHOR) > -1) {
                            go2 = true;
                        }
        ...
        if (curVersion == 57) {
            curVersion++;
            try (Cursor results57 = db.rawQuery(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='" + TBL_AUTHORS + '\'',
                    null)) {
            ...
            try (Cursor results = db.rawQuery(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='" + TBL_BOOKSHELF + '\'',
                    null)) {
                if (results.getCount() == 0) {
                    //table does not exist
            ...
            try (Cursor results = db.rawQuery(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='" + TBL_SERIES + '\'',
                    null)) {
                if (results.getCount() == 0) {
                    //table does not exist
                    db.execSQL(DATABASE_CREATE_SERIES_82);
                    try (Cursor results2 = db.rawQuery("SELECT * FROM " + TBL_BOOKS, null)) {
                        if (results2.getCount() > 0) {
                            if (results2.getColumnIndex(OLD_KEY_SERIES) > -1) {
                                db.execSQL(
                                        "INSERT INTO " + TBL_SERIES + " (" + DOM_SERIES_NAME + ") "
                                                + "SELECT name FROM ("
                                                + "    SELECT Upper(" + OLD_KEY_SERIES + ") " + COLLATION + " AS ucName, "
                                                + "    max(" + OLD_KEY_SERIES + ')' + COLLATION + " AS name FROM " + TBL_BOOKS
                                                + "    WHERE Coalesce(" + OLD_KEY_SERIES + ",'') <> ''"
            ...
            try (Cursor results = db.rawQuery(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='" + TBL_BOOKS + '\'',
                    null)) {
                if (results.getCount() == 0) {
                    //table does not exist
                    db.execSQL(DATABASE_CREATE_BOOKS_63);
            ...
            try (Cursor results = db.rawQuery(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='" + TBL_LOAN + '\'',
                    null)) {
                if (results.getCount() == 0) {
                    //table does not exist
                    db.execSQL(DATABASE_CREATE_BOOK_LOAN_82);
            ...
            try (Cursor results = db.rawQuery(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='" + TBL_TOC_ENTRIES + '\'',
                    null)) {
                if (results.getCount() == 0) {
                    //table does not exist
                    db.execSQL(DATABASE_CREATE_ANTHOLOGY_82);
            ...
            try (Cursor results = db.rawQuery(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='" + TBL_BOOK_BOOKSHELF + '\'',
                    null)) {
                if (results.getCount() == 0) {
                    //table does not exist
                    db.execSQL(DATABASE_CREATE_BOOK_BOOKSHELF_82);
            ...
            try (Cursor results = db.rawQuery(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='" + TBL_BOOK_SERIES + '\'',
                    null)) {
                if (results.getCount() == 0) {
                    //table does not exist
                    db.execSQL(DATABASE_CREATE_BOOK_SERIES_54);
                    db.execSQL("INSERT INTO " + TBL_BOOK_SERIES + " (" + DOM_FK_BOOK_ID + ',' +
                                       DOM_FK_SERIES_ID + ',' + DOM_BOOK_SERIES_NUM + ',' + DOM_BOOK_SERIES_POSITION + ") "
                                       + "SELECT DISTINCT b." + DOM_PK_ID + ", s." + DOM_PK_ID + ", b." + DOM_BOOK_SERIES_NUM + ", 1"
                                       + " FROM " + TBL_BOOKS + " b "
                                       + " JOIN " + TBL_SERIES + " s ON Upper(s." + DOM_SERIES_NAME + ") = Upper(b." + OLD_KEY_SERIES + ')' + COLLATION
                                       + " WHERE Coalesce(b." + OLD_KEY_SERIES + ", '') <> ''");
            ...
            try (Cursor results = db.rawQuery(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='" + TBL_BOOK_AUTHOR + '\'',
                    null)) {
                if (results.getCount() == 0) {
                    //table does not exist
                    db.execSQL(DATABASE_CREATE_BOOK_AUTHOR);
            ...
            try (Cursor results = db.rawQuery("SELECT * FROM " + TBL_BOOKS, null)) {
                if (results.getCount() > 0) {
                    if (results.getColumnIndex(OLD_KEY_SERIES) > -1) {
                        String tmpFields = DOM_PK_ID + "," /* + KEY_AUTHOR + "," */ + DOM_TITLE + ',' +
                                DOM_BOOK_ISBN + ',' + DOM_BOOK_PUBLISHER + ',' +
                                DOM_BOOK_DATE_PUBLISHED + ',' + DOM_BOOK_RATING + ',' + DOM_BOOK_READ +
                                /* "," + KEY_SERIES + */ ',' + DOM_BOOK_PAGES + ',' +
                                /* KEY_SERIES_NUM  "," + */ DOM_BOOK_NOTES + ',' +
                                DOM_BOOK_PRICE_LISTED + ',' + DOM_BOOK_ANTHOLOGY_BITMASK + ',' +
                                DOM_BOOK_LOCATION + ',' + DOM_BOOK_READ_START + ',' +
                                DOM_BOOK_READ_END + ',' + DOM_BOOK_FORMAT + ',' +
                                DOM_BOOK_SIGNED + ',' + DOM_BOOK_DESCRIPTION + ',' +
                                DOM_BOOK_GENRE;
                        db.execSQL(
                                "CREATE TABLE tmpBooks AS SELECT " + tmpFields + " FROM " + TBL_BOOKS);
                        db.execSQL("DROP TABLE " + TBL_BOOKS);
        ...
        if (curVersion == 58) {
            ...
            db.delete(TBL_LOAN.getName(),
                      "(" + DOM_FK_BOOK_ID + "='' OR " + DOM_FK_BOOK_ID + "=null OR " + DOM_LOANED_TO + "='' OR " + DOM_LOANED_TO + "=null) ",
                      null);
            mMessage += "New in v3.6\n\n";
            mMessage += "* The LibraryThing Key will be verified when added\n\n";
            mMessage += "* When entering ISBN's manually, each button with vibrate the phone" +
                    " slightly\n\n";
            mMessage += "* When automatically updating fields click on each checkbox twice" +
            ...
            mMessage += "* The Amazon search will now look for English, French, German, " +
                    "Italian and Japanese versions\n\n";
        }
        if (curVersion == 59) {
            curVersion++;
            mMessage += "New in v3.6.2\n\n";
            mMessage += "* Optionally restrict 'Sort By Author' to only the first Author" +
        ...
        if (curVersion == 60) {
            ...
            mMessage += "Hint: The export function will create an export.csv file " +
                    "on the sdcard\n\n";
            mMessage += "* You can crop cover thumbnails (both from the menu and after " +
            ...
            mMessage += "* Will check for network connection prior to searching for details " +
        ...
        if (curVersion == 61) {
            ...
            mMessage += "* Fixed several defects (including multiple author's and author" +
                    " prefix/suffix's)\n\n";
        ...
        if (curVersion == 63) {
            ...
            curVersion++;
            db.execSQL(
                    "UPDATE " + TBL_BOOKS + " SET " + DOM_BOOK_READ + "=0 WHERE " + DOM_BOOK_READ + "='f'");
            db.execSQL(
                    "UPDATE " + TBL_BOOKS + " SET " + DOM_BOOK_READ + "=1 WHERE " + DOM_BOOK_READ + "='t'");
        ...
        if (curVersion == 64) {
            ...
            curVersion++;
            db.execSQL(
                    "UPDATE " + TBL_BOOKS + " SET " + DOM_BOOK_SIGNED + "=0 WHERE " + DOM_BOOK_SIGNED + "='f'");
            db.execSQL(
                    "UPDATE " + TBL_BOOKS + " SET " + DOM_BOOK_SIGNED + "=1 WHERE " + DOM_BOOK_SIGNED + "='t'");
            db.execSQL(
                    "ALTER TABLE " + TBL_BOOKS + " Add " + DOM_BOOK_DATE_ADDED + " datetime"); // Just want a null value for old records
            db.execSQL("ALTER TABLE " + TBL_BOOKS + " RENAME TO books_tmp");
        ...
        if (curVersion == 68) {
            curVersion++;
            db.execSQL(
                    "ALTER TABLE " + TBL_BOOKS + " ADD " + DOM_BOOK_GOODREADS_BOOK_ID + " integer");
        }
        ...
        if (curVersion == 71) {
            ...
            mMessage += "New in v4.0 - Updates courtesy of (mainly) Philip Warner " +
            ...
            mMessage += "* Cover images now have globally unique names, and books have globally" +
                    " unique IDs, so sharing and combining collections is easy\n\n";
        ...
        if (curVersion == 74) {
            curVersion++;
            Prefs.getPrefs().edit().putBoolean(V74_PREF_AUTHOR_SERIES_FIX_UP_REQUIRED,
                                               true).apply();
            mMessage += "New in v4.0.3\n\n";
            mMessage += "* ISBN validation when searching/scanning and error beep when scanning " +
                    "(with preference to turn it off)\n\n";
            mMessage += "* 'Loaned' list now shows available books under the heading " +
                    "'Available'\n\n";
            ...
            mMessage += "Thank you to all those people who reported bugs and helped tracking " +
                    "them down.\n\n";
        }
        ...
        if (curVersion == 77) {
            ...
            mMessage += "* When adding books, scanning or typing an existing ISBN allows you" +
